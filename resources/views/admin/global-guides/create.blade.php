        ...
        <form action="{{ route('admin.global-guides.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="type" class="form-control @error('type') is-invalid @enderror" required>
                    <option value="usage" {{ old('type') == 'usage' ? 'selected' : '' }}>Quá trình sử dụng xe</option>
                    <option value="accident" {{ old('type') == 'accident' ? 'selected' : '' }}>Xử lý tai nạn</option>
                    <option value="insurance_rules" {{ old('type') == 'insurance_rules' ? 'selected' : '' }}>Quy tắc bảo hiểm</option>
